...
WebpackNotifierPlugin.prototype.compileMessage = function(stats) {
  ...
  if (error.error) message = "Error: " + message + error.error.toString();
...
WebpackNotifierPlugin.prototype.compilationDone = function(stats) {
  ...
  if (msg) {
    var contentImage =
      "contentImage" in this.options ? this.options.contentImage : DEFAULT_LOGO;

    if (this.options.growl) {
      new notifier.Growl({ host: this.options.host }).notify(
        objectAssign(
          {
            title: "Webpack",
            message: msg,
